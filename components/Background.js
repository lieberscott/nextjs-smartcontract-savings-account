...
export default function Background(props) {
  ...
  const { account } = useMoralis();
    ...
    <div className="w-full max-w-lg mt-8">      
      ...
      <p className="mb-3 font-light text-gray-500 dark:text-gray-400">The project is taken from an idea in an article written by Vitalik Buterin when he first introduced the Ethereum project in Bitcoin Magazine in 2014:</p>
      <p className="mb-3 font-light text-gray-500 dark:text-gray-400">"Suppose that Alice wants to store a large amount of money, but does not want to risk losing everything if her private key is lost or stolen. She makes a contract with Bob, a semi-trustworthy bank, with the following rules: Alice is allowed to withdraw up to 1 per day, Alice with Bob approval can withdraw any amount, and Bob alone can withdraw up to 0.05 per day. Normally, Alice will only need small amounts at a time, and if Alice wants more she can prove her identity to Bob and make the withdrawal. If Alice's private key gets stolen, she can run to Bob and move the funds into another contract before the thief gets away with more than 1 of the funds. If Alice loses her private key, Bob will eventually be able to recover her funds. And if Bob turns out to be evil, Alice can withdraw her own funds twenty times faster than he can. In short, all of the security of traditional banking, but with almost none of the trust."</p>
