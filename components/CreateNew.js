...
export default function CreateNew(props) {
    ...
    const handleBackupAccountLimit = (amountInEth) => {
      const isValid = !isNaN(parseInt(amountInEth)) && Number(parseInt(amountInEth)) > 0;
      const wei = isValid ? ethers.utils.parseUnits(amountInEth, "ether").toString() : ""
      const amt = isValid ? amountInEth : ""
        ...
        <div class="mt-6">
          ...
          <div class="flex flex-wrap -mx-3 mb-6">
              <div class="w-full px-3">
                ...
                <p class="text-gray-600 text-xs italic">This is stored on-chain so you can identify it. The shorter, the cheaper.</p>
                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-password" type="text" placeholder="Scott's Account" onChange={(e) => setName(e.target.value)} />
              </div>
            </div>


            <div class="flex flex-wrap -mx-3 mb-6">
              <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0 align-baseline">
                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-first-name">
                  Primary Account Address
                </label>
                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-primary-address" type="text" placeholder={account} onChange={(e) => setMainAccount(e.target.value) } />
              ...
              <div class="w-full md:w-1/2 px-3">
                ...
                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-primary-eth-limit" type="number" min="0" placeholder="0.5" value={ mainAccountWithdrawalLimit_Eth } onChange={(e) => handleMainAccountLimit(e.target.value) } />
              </div>
            </div>
            ...
            <div class="flex flex-wrap -mx-3 mb-6">
              <div class="w-full md:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-red-500 rounded py-3 px-4 mb-3 leading-tight focus:outline-none focus:bg-white" id="grid-backup-address" type="text" placeholder={account} onChange={(e) => setBackupAccount(e.target.value) } />
                <p class="text-red-500 text-xs italic">Please fill out this field.</p>
              </div>
              <div class="w-full md:w-1/2 px-3">
                <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2" for="grid-last-name">
                ...
                <input class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" id="grid-backup-eth-limit" type="number" min="0" placeholder="0.02" value={ backupAccountWithdrawalLimit_Eth } onChange={(e) => handleBackupAccountLimit(e.nativeEvent.value) } />
              </div>
            </div>
